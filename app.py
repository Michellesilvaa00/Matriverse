"""
MATRIVERSE · app.py  —  versão produção (Render)
Alterações em relação ao dev:
  - Serve os arquivos estáticos do frontend (pasta /static)
  - SECRET_KEY lida da variável de ambiente (mais seguro)
  - SESSION_COOKIE_SECURE=True  (obrigatório com HTTPS)
  - CORS liberado para a URL do Render + localhost
  - Banco SQLite salvo em /tmp/matriverse.db (disco efêmero do Render free tier)
  - Debug desligado em produção
  - Inicialização do banco no import (não só em __main__)
"""
...
# ── Configuração ─────────────────────────────────────────
RENDER_URL = os.environ.get("RENDER_EXTERNAL_URL", "")  # preenchido pelo Render
SECRET_KEY = os.environ.get("SECRET_KEY", "matriverse-secret-dev-2026")

# Em produção o Render usa /tmp para escrita (plano gratuito não tem disco persistente)
DB = os.path.join("/tmp", "matriverse.db")
...
# CORS: aceita o próprio domínio do Render + localhost para dev
_origins = ["http://localhost:8080", "http://127.0.0.1:8080", "null"]
if RENDER_URL:
    _origins.append(RENDER_URL)
...
QUESTOES = {
    ...
    3: [
        {"id":301,"enunciado":"Qual o resultado da multiplicação das matrizes?\n\nA = [ 1  2 ]\n    [ 3  4 ]\n\nB = [ 2  0 ]\n    [ 1  5 ]","alternativas":["[[4,10],[10,20]]","[[4,8],[10,20]]","[[5,10],[10,20]]","[[4,10],[8,20]]"],"correta":0},
        {"id":302,"enunciado":"Multiplique as matrizes:\n\nA = [ 2  1  3 ]\n    [ 0  4  2 ]\n    [ 1  5  1 ]\n\nB = [ 1  2  0 ]\n    [ 3  1  4 ]\n    [ 2  5  1 ]","alternativas":["[[11,20,7],[16,14,18],[18,12,21]]","[[10,20,7],[16,14,18],[18,12,21]]","[[11,18,7],[16,14,18],[18,12,21]]","[[11,20,9],[16,14,18],[18,12,21]]"],"correta":0},
        {"id":303,"enunciado":"Se A tem dimensão 2×3 e B tem dimensão 3×3,\nqual será a dimensão da matriz resultado A × B?","alternativas":["3×2","2×2","2×3","3×3"],"correta":2},
        {"id":304,"enunciado":"Calcule o elemento C₂,₃ da multiplicação C = A × B:\n\nA = [  1   0   2 ]\n    [ -1   3   1 ]\n    [  2   1   0 ]\n\nB = [ 3  1  2 ]\n    [ 2  0  1 ]\n    [ 1  4  3 ]","alternativas":["2","3","4","5"],"correta":2},
        {"id":305,"enunciado":"Calcule 2A + 3B − C:\n\nA = [ 1  2 ]    B = [ 2  1 ]    C = [ 4  5 ]\n    [ 3  4 ]        [ 0  3 ]        [ 6  7 ]","alternativas":["[[4,2],[0,10]]","[[4,0],[0,10]]","[[2,2],[0,10]]","[[4,2],[2,10]]"],"correta":0},
    ],
    4: [
...
@app.route("/")
def index():
    """Serve o landing.html como página inicial."""
...
def entrar_sala():
    ...
    if not sala:
        conn.close()
        return jsonify({"erro": "Código de sala inválido."}), 404
    try:
...
def questoes(mundo):
    if mundo not in QUESTOES:
        return jsonify({"erro": "Mundo não encontrado."}), 404
    return jsonify([{"id": q["id"], "enunciado": q["enunciado"], "alternativas": q["alternativas"]}
                    for q in QUESTOES[mundo]])
...
def responder():
    ...
    if mundo not in QUESTOES:
        return jsonify({"erro": "Mundo inválido."}), 400
    ...
    return jsonify({
        ...
        "texto_certo":  questao["alternativas"][questao["correta"]]
    })
...
def salvar_progresso():
    ...
    d        = request.json or {}
    mundo    = d.get("mundo")
    estrelas = d.get("estrelas", 0)
    concluido = int(d.get("concluido", False))
    conn = get_db()
    existente = conn.execute("SELECT estrelas FROM progresso WHERE usuario_id=? AND mundo=?",
                             (u["id"], mundo)).fetchone()
    if existente:
...
# ── Inicialização ─────────────────────────────────────────
# Roda tanto no __main__ (dev) quanto no import pelo gunicorn (prod)
init_db()
