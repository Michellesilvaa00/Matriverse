...
app = Flask(__name__)
app.secret_key = "matriverse-secret-2026"
app.config.update(
    SESSION_COOKIE_SAMESITE="None",
    SESSION_COOKIE_SECURE=False,   # True em produção com HTTPS
)
CORS(app, supports_credentials=True, origins=["http://localhost:8080", "http://127.0.0.1:8080", "null"])
...
QUESTOES = {
    ...
    {
    "id":303,
    "enunciado":"""Levando em consideração:
    ...
    Qual será a dimensão da matriz resultado multiplicada?""",
    "alternativas":[
        "3x2",
        "2x2",
        "2x3",
    ...
    {
    "id":304,
    "enunciado":"""Calcule o elemento C₂,₃ da multiplicação:
...
def usuario_logado():
    # Aceita cookie de sessão OU header X-User-Id (fallback cross-origin)
...
def criar_sala():
    ...
    d = request.json or {}
    # Aceita qualquer nome — sem restrição de caracteres
    nome = (d.get("nome") or "").strip()
